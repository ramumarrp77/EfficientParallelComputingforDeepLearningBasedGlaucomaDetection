"""
Script Name: ParallelProcessing/gpus_with_DDP/model.py
Author: Ram Kumar Ramasamy Pandiaraj
Description:
This script defines a Convolutional Neural Network (CNN) model architecture for medical image classification. 
The model is designed to classify glaucoma images using the 'GlaucomaDataset' dataset. 
It includes several custom blocks such as Convolutional, SE (Squeeze-and-Excitation), Residual, and MultiDilated blocks to enhance feature learning and capture multi-scale information. 
The dataset is pre-processed using the `get_medical_transforms` function for augmentation, and the model uses Cross-Entropy Loss for classification and AdamW optimizer for training.
Function Arguments:
- X (array): The input data (image data).
- y (array): The corresponding labels for the input data.
- transform (callable): A function/transform that takes in an image and returns a transformed version. Defaults to None.
- num_classes (int): The number of output classes. Defaults to 2 (for binary classification).
- base_filters (int): The number of filters in the first convolution layer. Defaults to 64.
"""
...
class GlaucomaDataset(Dataset):
    """
    Custom dataset class for loading the Glaucoma dataset.

    Arguments:
    - X (array): The image data.
    - y (array): The corresponding labels.
    - transform (callable, optional): A function/transform that takes an image and returns a transformed image.
    """
...
class ConvBlock(nn.Module):
    """
    Convolutional Block that applies convolution, batch normalization, and ReLU activation.
...
class SEBlock(nn.Module):
    ...
    - reduction (int, optional): The reduction ratio for the Squeeze-and-Excitation operation. Defaults to 16.
    """
...
class ResidualBlock(nn.Module):
    """
    Residual Block with two convolution layers and an optional Squeeze-and-Excitation (SE) block.

    Arguments:
    - in_channels (int): The number of input channels.
    - out_channels (int): The number of output channels.
    - stride (int, optional): The stride of the first convolution. Defaults to 1.
    - downsample (callable, optional): A downsampling function for matching dimensions. Defaults to None.
    """
...
class MultiDilatedBlock(nn.Module):
    """
    Multi-Dilated Convolution Block to capture multi-scale contextual information.
...
class MedicalCNN(nn.Module):
    """
    A Convolutional Neural Network for glaucoma image classification.
...
def get_medical_transforms():
    """
    Returns the data augmentation and normalization transformations for training and validation data.

    Returns:
    - train_transform (callable): The transformation pipeline for training data.
    - val_transform (callable): The transformation pipeline for validation data.
    """
